...
(function () {
  ...
  const els = {
    ...
    // عناصر المودال (نسخة UDSAL الجديدة)
  ...
  // لو ما وجدنا عناصر أساسية نوقف السكربت
  ...
  function bindEvents() {
    ...
    els.regionChips.forEach((chip) => {
      chip.addEventListener("click", () => {
        const regionAttr = chip.getAttribute("data-region") || "all";
        state.activeRegion = regionAttr;
    ...
    // تفويض الكليك على زر Open في الكروت
...
<article class="result-card">
  ...
  <div class="result-header">
    <div class="result-title">
      ...
      ${
        svc.subtitle
          ? `<span class="sub">${escapeHtml(svc.subtitle)}</span>`
          : ""
      }
    </div>

    <div class="result-pills">
      <span class="pill region">${escapeHtml(svc.region_label || "")}</span>
      ${
        svc.regionBadge
          ? `<span class="pill">${escapeHtml(svc.regionBadge)}</span>`
          : ""
      }
      ${
        statusLabel
  ...
  <p class="result-body">${escapeHtml(svc.description || "")}</p>
  ...
  <div class="result-meta-row">
    ...
    ${
      hasUrl
        ? `<button class="open-btn" type="button" data-open-service="${svc.id}">
             Open
           </button>`
        : `<button class="open-btn secondary" type="button" disabled>
             قريباً
           </button>`
    }
  </div>
  ...
  ${
    tags
      ? `<div class="tags-wrapper" style="margin-top:6px;">${tags}</div>`
      : ""
  }
