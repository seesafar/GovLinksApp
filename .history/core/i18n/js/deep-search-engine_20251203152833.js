...
// سلوك الفلاتر: واحد فقط في كل مرة (Exclusive)
filterChips.forEach((chip) => {
  chip.addEventListener("click", () => {
    const key = chip.dataset.filter;

    // إذا كان الفلتر نفسه شغّال → نطفيه
    if (chip.classList.contains("is-on")) {
      chip.classList.remove("is-on");
      activeFilters.clear();
    } else {
      // نطفي كل الفلاتر الأخرى
      filterChips.forEach((c) => c.classList.remove("is-on"));
      activeFilters.clear();

      // نشغّل هذا الفلتر فقط
      chip.classList.add("is-on");
      activeFilters.add(key);
    }

    // إعادة البحث أو التحديث
    if (typeof runSearch === "function") {
      runSearch();
    } else if (typeof applySearch === "function") {
      applySearch();
    } else if (typeof updateSearchResults === "function") {
      updateSearchResults();
    } else {
      console.warn("No search update function found for filters.");
    }
  });
});
1) const activeFilters = new Set();
2) const filterChips = document.querySelectorAll(".filter-chip");

⭐ هنا ألصق نظام الفلاتر كامل ⭐

3) ثم بعدها باقي الأكواد (input, region tabs, applySearch...)
