...
function applyFiltersAndRender() {
  ...
  const hasRegionFilter = activeRegion !== "all";
  const hasCategoryFilter = state.activeFilters.size > 0;
  const q = (state.query || "").toLowerCase().trim();
  const hasQuery = !!q;

  let filtered = state.allServices;

  // لو فيه أي فلتر فعلي (دولة / تصنيف / بحث) نطبّق الفلاتر
  if (hasRegionFilter || hasCategoryFilter || hasQuery) {
    filtered = state.allServices.filter((svc) => {
      // فلتر الدولة
      if (hasRegionFilter) {
    ...
    // ما فيه لا دولة محددة ولا فلتر ولا بحث → اعرض كل الخدمات
    filtered = state.allServices.slice();
  }
