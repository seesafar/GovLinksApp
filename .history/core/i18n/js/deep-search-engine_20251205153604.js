...
(function () {
  // 📂 مسار ملف JSON (لوكال vs GitHub Pages)
  const isGithub = location.hostname.includes("github.io");

  const SERVICES_JSON_URL = isGithub
    ? "/GovLinksApp/core/i18n/data/services-data.json"   // على GitHub Pages
    : "../core/i18n/data/services-data.json";            // على Live Server المحلي
  ...
  // اسم لطيف لكل category (يظهر في المودال)
  ...
  // تطبيع النص للبحث (يدعم العربي والإنجليزي)
  function normalizeText(text) {
    return String(text || "")
      .toLowerCase()
      // أشكال الألف -> ا
      .replace(/[أإآ]/g, "ا")
      // تاء مربوطة -> ه (اختياري لكنه مفيد)
      .replace(/ة/g, "ه")
      // ألف مقصورة -> ي
      .replace(/ى/g, "ي")
      // حذف التطويل وبعض العلامات
      .replace(/ـ/g, "")
      // استبدال أي رموز غريبة بمسافة
      .replace(/[^\w\s\u0600-\u06FF]/g, " ")
  ...
  // لو ما لقينا عناصر أساسية، نوقف السكربت بهدوء
  ...
  function applyFiltersAndRender() {
    ...
    // لو فيه فلتر تصنيف مختار (منصات موحدة / عدل / هوية ...)
    let activeFilterKey = null;
    let activeFilterValue = null;
    if (state.activeFilters.size > 0) {
      activeFilterKey = Array.from(state.activeFilters)[0];
      activeFilterValue = (FILTER_MAP[activeFilterKey] || "").toLowerCase();
    }

    state.filtered = state.allServices.filter((svc) => {
      // 1) فلتر الدولة
      const svcRegion = normalizeRegionValue(svc.region || "all");
      if (activeRegion !== "all" && svcRegion !== activeRegion) {
        return false;
      }
      ...
      if (activeFilterValue) {
        ...
        if (!cat.includes(activeFilterValue)) {
          return false;
        }
      }
  ...
  // =========================
  // 4) عرض النتائج (كروت طويلة service-card)
  // =========================
    ...
    if (!list.length) {
      ...
      if (els.emptyState) {
        ...
        els.emptyState.textContent =
          ...
          state.activeFilters.size
            ? "لا توجد نتائج مطابقة حالياً… جرّب كلمة أخرى، أو غيّر الدولة أو الفلتر."
      ...
      const cardsHtml = list
        .map((svc) => {
          ...
          // حالة الخدمة (للتلوين لو حبيت تستفيد منها لاحقاً)
          let statusClass = "";
          let statusLabel = "";
          switch (svc.status) {
            case "live":
...
<article class="service-card">
  <header class="service-card-header">
    <div class="service-region">
      ...
      ${
        statusLabel
          ? `<span class="region-badge ${statusClass}">${escapeHtml(
              statusLabel
            )}</span>`
          : ""
      }
    ...
    ${
      svc.subtitle
        ? `<p class="service-subtitle">${escapeHtml(svc.subtitle)}</p>`
        : ""
    }
  </header>
...
`;
  ...
  function openServiceModal(svc) {
    ...
    // لو ما في مودال مخصص، نفتح الرابط مباشرة
    if (!els.modal) {
    ...
    if (els.modalRegion) els.modalRegion.textContent = svc.region_label || "";
  ...
  // 🔐 دالة لتأمين النص داخل الـ HTML
