...
(function () {
  ...
  let allServices = [];
  let currentQuery = "";
  let currentCategory = "all"; // تصنيف الفلتر الحالي
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch(SERVICES_JSON_URL)
      ...
      .then((data) => {
        ...
        initSearchInput();     // تفعيل البحث
        initCategoryFilters(); // تفعيل فلاتر التصنيف
      })
      .catch((err) => {
  ...
  // ========= أدوات Debug في الـ Console =========
  window.__deep = {
    getCount: () => allServices.length,
    query(q) {
      currentQuery = String(q || "").toLowerCase().trim();
      renderResults();
      return `Filtered by: ${q}`;
    },
    setCategory(cat) {
      currentCategory = String(cat || "all");
      renderResults();
  ...
  // ========= فلترة البيانات حسب التصنيف + النص =========
    ...
    if (currentCategory && currentCategory !== "all") {
      list = list.filter((svc) => {
        if (Array.isArray(svc.categories)) {
          return svc.categories.includes(currentCategory);
        }
  ...
  // ========= حماية من إدخال HTML من البيانات =========
