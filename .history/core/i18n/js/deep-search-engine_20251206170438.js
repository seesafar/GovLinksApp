alert("✅ deep-search-engine.js تم تحميله بنجاح");
...
function updateSuggestions() {
  ...
  // نبحث عن خدمات تطابق البحث (مع مراعاة الدولة)
  ...
  matches.forEach((svc) => {
    ...
    regionSpan.className = "search-suggestion-region";
    regionSpan.textContent = regionLabel(svc.region);

    main.appendChild(nameSpan);
    main.appendChild(regionSpan);

    const urlSpan = document.createElement("span");
    urlSpan.className = "search-suggestion-url";
    urlSpan.textContent = svc.url;
    ...
    btn.addEventListener("click", () => {
      // نحط اسم الخدمة في خانة البحث
      state.query = svc.name;
      if (els.searchInput) {
        els.searchInput.value = svc.name;
      }

      // نطبّق الفلاتر ونظهر النتائج
      applyFilters();
      renderResults();
...
if (els.searchInput) {
  els.searchInput.addEventListener("input", (ev) => {
    ...
    // فلترة النتائج وعرضها
    applyFilters();
    renderResults();

    // 🔍 تحديث قائمة الاقتراحات الذكية
    updateSuggestions();
  });
}
...
document.addEventListener("click", (ev) => {
  ...
  const clickedInsideSearch =
    els.suggestions.contains(target) ||
    (els.searchInput && els.searchInput.contains(target));
