  ...
  async function loadServices() {
    try {
      ...
      els.filterChips.forEach((chip) => {
       ...
       // =========================
  // 3) تطبيق الفلاتر (نسخة مبسّطة ومضمونة)
  // =========================
  function applyFiltersAndRender() {
    let list = state.allServices.slice();

    // 1) فلتر المنطقة
    if (state.activeRegion !== "all") {
      list = list.filter((svc) => svc.region === state.activeRegion);
    }

    // 2) فلتر التصنيف (لو فيه فلتر شغّال)
    if (state.activeFilters.size > 0) {
      const neededCategories = Array.from(state.activeFilters).map(
        (key) => FILTER_MAP[key] || key
      );
      list = list.filter((svc) =>
        neededCategories.includes(svc.category)
      );
    }
