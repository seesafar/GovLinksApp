...
// قراءة بيانات الخدمات + بناء نتائج البحث
...
(function () {
  ...
  function initSearchInput() {
    const input = document.querySelector(".search-input-wrap input[type='search']");
    if (!input) return;
  ...
  function renderResults() {
    ...
    Object.keys(grouped).forEach((groupKey) => {
      ...
      groupItems.forEach((svc) => {
        ...
        if (status === "warning") {
          statusPill = '<span class="pill status-risk">Warning · Not official</span>';
        } else if (status === "soon") {
          statusPill = '<span class="pill status-soon">Coming soon</span>';
        } else {
          statusPill = '<span class="pill status-ok">Official · Live</span>';
        }
          ...
          <article class="result-card">
            <div class="result-header">
              <div class="result-title">
                ...
                ${svc.subtitle ? `<span class="sub">${escapeHtml(svc.subtitle)}</span>` : ""}
              </div>
              <div class="result-pills">
                ${svc.region_label ? `<span class="pill region">${escapeHtml(svc.region_label)}</span>` : ""}
                ${statusPill}
              </div>
            </div>

            ${svc.description ? `<p class="result-body">${escapeHtml(svc.description)}</p>` : ""}

            <div class="result-meta-row">
              ${svc.url ? `<span class="result-url">${escapeHtml(svc.url)}</span>` : "<span></span>"}
              <button class="open-btn${status === "warning" ? " secondary" : ""}" type="button">
                ${svc.cta ? escapeHtml(svc.cta) : status === "warning" ? "Show details" : "Open"}
              </button>
            </div>
          </article>
        `;
  ...
  // -------------------------------------------------------------------
  // ⭐ Debug Helpers — نستخدمها من Console
  // -------------------------------------------------------------------
  window.__deep = {
    getCount: () => allServices.length,
    getAll: () => allServices,
    query: (txt) => {
      currentQuery = (txt || "").toLowerCase();
      renderResults();
      return `Filtered by: ${txt}`;
    }
  };
